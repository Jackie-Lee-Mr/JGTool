...
-(void)initShow5{
    CGFloat f = 10; //设置间隔
    CGFloat width = ((self.frame.size.width-(self.num)*f)/self.num);
...
{
    if (![JGCommonTools isAllNum:textField.text]) {
         textField.text = @"";
        [QJCustomHUD showError:@"粘贴不合法字符"];
        return;
    }
